...
from helpers import apology, login_required, lookup, usd, monetary
...
def index():
    ...
    for i in range(len(shares_dict)):
        db.execute("UPDATE shares SET stock_price = :stock_price, shares_value = :shares_value WHERE user_id = :user_id AND stock_symbol = :stock_symbol",
            ...
            stock_price = monetary(lookup(shares_dict[i]["stock_symbol"])['price']),
            shares_value = monetary(shares_dict[i]["shares"] * lookup(shares_dict[i]["stock_symbol"])['price']))

    # Display user portfolio
    user_cash = monetary(db.execute("SELECT * FROM users WHERE id = :user_id", user_id = session["user_id"])[0]["cash"])
    sum_shares = db.execute("SELECT SUM(shares_value) FROM shares WHERE user_id = :user_id", user_id = session["user_id"])[0]["SUM(shares_value)"]
    if not sum_shares:
    ...
    if request.method == "GET":
        ...
        if len(user_shares) == 1:
            ...
            db.execute("INSERT INTO shares (user_id, stock_symbol, stock_name, shares, stock_price, shares_value) VALUES (:user_id, :stock_symbol, :stock_name, :shares, :stock_price, :shares_value)",
                        ...
                        stock_price = monetary(lookup(request.form.get("symbol"))['price']),
                        shares_value = monetary(requested_shares * lookup(request.form.get("symbol"))['price']))

        # Update user balance
        db.execute("UPDATE users SET cash = :balance WHERE id = :id", balance =  (user_cash - cost), id = session["user_id"])
        return redirect("/")
...
def sell():
    if request.method == "GET":
        ...
        user_shares = db.execute("SELECT * FROM shares WHERE user_id = :user_id AND stock_symbol =:stock_symbol", user_id = session["user_id"], stock_symbol = request.form.get("symbol"))
        if len(user_shares) != 1:
        ...
        if updated_shares == 0:
            ...
            db.execute("UPDATE shares SET shares = :shares, stock_price = :stock_price, shares_value = :shares_value WHERE user_id = :user_id AND stock_symbol = :stock_symbol",
                        shares = updated_shares,
                        stock_price = monetary(lookup(request.form.get("symbol"))['price']),
                        shares_value = monetary(updated_shares * lookup(request.form.get("symbol"))['price']),
                        user_id = session["user_id"],
                        stock_symbol = request.form.get("symbol"))
