...
def index():
    ...
    user_value = user_cash + db.execute("SELECT SUM(shares_value) FROM shares WHERE user_id = :user_id", user_id = session["user_id"])[0]["SUM(shares_value)"]
...
def buy():
    if request.method == "GET":
        ...
        if len(stocks) == 1:
            ...
            db.execute("INSERT INTO shares (user_id, stock_symbol, stock_name, shares) VALUES (:user_id, :stock_symbol, :stock_name, :shares)",
                        user_id = session["user_id"],
                        stock_symbol = request.form.get("symbol"),
                        stock_name = lookup(request.form.get("symbol"))['name'],
